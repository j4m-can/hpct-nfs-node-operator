...
NFS node operator acting as principal for subordinate charms.
...
```
juju deploy ./hpct-nfs-node-operator_ubuntu-22.04-amd64.charm
```

Assuming a `hpct-xxx-subordinate-operator` has been deployed:

```
juju relate hpct-nfs-node-operator hpct-xxx-subordinate-operator
```
